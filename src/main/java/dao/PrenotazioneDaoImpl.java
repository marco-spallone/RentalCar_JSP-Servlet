...
public class PrenotazioneDaoImpl implements PrenotazioneDao{
    ...
    public List<Prenotazione> prenotazioniDate(Date inizio, Date fine) {
        try(Session session= HibernateUtil.getSessionFactory().openSession()){
            return session.createQuery("SELECT a FROM Prenotazione a WHERE (a.dataInizio BETWEEN :inizio AND :fine) OR (a.dataFine BETWEEN :inizio AND :fine)" +
                            "OR (a.dataInizio<=:inizio AND a.dataFine>=:fine)",
